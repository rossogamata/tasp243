...
> **Місце в модулі:** етап 1, мережева основа Cloud Operations Portal.
>
> **Розподіл роботи:** теорія і вимоги опрацьовуються на лекції; наведені нижче команди викладач демонструє на груповому занятті. На практичному курсант за власною схемою самостійно створює мережу та документує результат.

## Результат етапу

Після практичного заняття у курсанта має бути мережевий фундамент майбутнього порталу: VPC, дві subnet у різних AZ, маршрут до Internet Gateway, правила NACL і Security Group, а також зафіксований стан ресурсів для наступного заняття.

### Підготовка курсанта до практики

До початку виконання команд підготуйте:

1. схему VPC із CIDR і двома AZ;
2. таблицю дозволених портів із поясненням, де діє кожне правило;
3. план іменування ресурсів і тегів;
4. `state/environment.example.env` без реальних ID ресурсів.

### Завдання практичного заняття

Створіть мережеву основу для Cloud Operations Portal, перевірте маршрутизацію та доступність тестового EC2, після чого оформіть схему і таблицю створених ресурсів. Не копіюйте команди механічно: спочатку визначте, який ресурс вони створюють і як він буде використаний на Lesson1_3.

**Контрольна точка:** наступне заняття повинно використовувати цю VPC, а не створювати нову.
