...
> **Місце в модулі:** етап 2, обчислювальний шар Cloud Operations Portal.
>
> **Попередня умова:** завершений [етап мережі](../Lesson1_2/README.md). Це заняття продовжує вже створену VPC і не повинно створювати нову мережу.
>
> **Наступні етапи:** S3 і CloudFront розглядаються в [Lesson1_4](../Lesson1_4/README.md), Lambda — у [Lesson1_5](../Lesson1_5/README.md), фінальний аудит — у [Lesson1_6](../Lesson1_6/README.md).
>
> **Розподіл роботи:** лекція пояснює моделі сервісів і вимоги; команди нижче викладач демонструє на груповому занятті; на практичному курсант самостійно розгортає обчислювальний шар EC2 і ALB за власною архітектурною схемою.

## Результат етапу

До кінця практичного етапу Cloud Operations Portal має працювати поверх VPC з Lesson1_2: два EC2 у різних AZ обслуговуються через ALB. S3, CloudFront і Lambda додаються на наступних заняттях.

### Підготовка до практики

Перед практичним заняттям підготуйте:

1. оновлену схему, у якій використано VPC, subnet і Security Group з Lesson1_2;
2. таблицю `компонент -> сервіс AWS -> причина вибору`;
3. план перевірок: прямий EC2, ALB, CloudFront, Lambda;
4. план cleanup для ресурсів, які створюються на цьому етапі.

Для демонстрації викладач може експортувати ID ресурсів у змінні. Під час самостійної практики курсант повинен відновити їх із власного стану, а не копіювати значення з прикладу.
...
> На цьому етапі ми додаємо до VPC з Lesson1_2 S3-бакет, CloudFront-дистрибуцію, EC2-сервери, ALB та Lambda-функцію. Route 53 показано як наступний крок: DNS-зона потребує доменного імені.
...
## 🌐 Крок 1 — Перевірка мережі з Lesson1_2

У цьому занятті ми не створюємо нову VPC і не використовуємо default VPC. Викладач показує команди перевірки вже наявної інфраструктури з Lesson1_2. Перед запуском наступних кроків потрібно отримати `VPC_ID`, `SUBNET_A_ID`, `SUBNET_B_ID` і `SG_ID` з попереднього стану.

```bash
export AWS_REGION=${AWS_REGION:-us-east-1}
export VPC_ID="vpc-..."
export SUBNET_A_ID="subnet-..."
export SUBNET_B_ID="subnet-..."
export SG_ID="sg-..."
...
> Значення `vpc-...`, `subnet-...` і `sg-...` є заповнювачами. На практиці використовуйте власні ID з Lesson1_2.

### Демонстраційний блок викладача

Наведені нижче команди створення VPC, IGW, subnet і route table є поясненням повного процесу та можуть бути виконані викладачем у чистому навчальному середовищі. Курсант не повторює цей блок у вже підготовленому проєкті, інакше буде створено другу незалежну мережу.
